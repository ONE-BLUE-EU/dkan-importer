...
fn test_ammonium_null_value_acceptance() {
    ...
    println!("Generated Schema for Ammonium test:");
    ...
    if !is_valid {
        println!("Validation errors for row 2:");
        for error in validator.validator.iter_errors(&test_data_row2) {
            println!("  - {}", error);
        }
    }
    assert!(
    ...
    println!(
        "✅ Regression test passed: Ammonium null values are now accepted for non-mandatory fields"
    );
    println!("✅ The original error 'Type mismatch at row[2]./Ammonium (μmol~1L): expected number, got null' is now resolved");
}
...
fn test_excel_cell_to_null_conversion() {
    ...
    println!("✅ Excel cell conversion test passed: Empty cells and strings convert to null for non-mandatory fields");
}
